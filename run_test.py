...
# Initialize a dictionary to track browser status
browser_status = {browser: True for browser in browsers}

def get_free_browser():
    """Returns an available browser, or None if all are busy."""
...
def run_test_suite(test_suite, browser):
    """Runs a test suite on a specified browser."""
    ...
    browser_status[browser] = True

def run_in_parallel(suites, browsers):
    """Run test suites in parallel, each on an available browser."""
    with ThreadPoolExecutor(max_workers=len(browsers)) as executor:
        futures = []
        for suite in suites:
            browser = get_free_browser()
            if browser:
                browser_status[browser] = False  # Mark browser as busy
                print(f"Starting {suite} on {browser}")
                future = executor.submit(run_test_suite, suite, browser)
                futures.append(future)
            else:
                # Wait for at least one future to complete if no browsers are free
                done_futures = [f for f in futures if f.done()]
                if done_futures:
                    for done_future in done_futures:
                        futures.remove(done_future)
                    # Re-check browser availability
                    browser = get_free_browser()
                    if browser:
                        print(f"Re-assigning {suite} to free browser {browser}")
                        future = executor.submit(run_test_suite, suite, browser)
                        futures.append(future)
